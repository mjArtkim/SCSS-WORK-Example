...
이 프로젝트는 돌보미 사용자 화면과 구인구직 화면을 제작하면서 **Figma로 화면을 기획하고 재사용 가능한 UI를 설계한 뒤, SCSS로 스타일 구조를 체계화한 프론트엔드 작업**입니다. 공통 값과 반복 패턴을 분리하고, 화면별 스타일 책임을 나누는 방식에 초점을 두었습니다.
...
현재 이 저장소에는 전체 서비스 코드가 아니라 제가 직접 작업한 코드만 정리했습니다. 컨트롤러와 관련 Java 파일도 일부 작업했지만, 컨트롤러 외 Java 영역은 깊게 관여한 범위가 아니므로 포함하지 않았습니다.
...
| 핵심 작업 | **Figma 기반 UI 기획/설계**, **SCSS 기반 스타일 구조화** |
| 디자인 도구 | **Figma**, Adobe Photoshop, Adobe Illustrator |
| 화면 구성 | Thymeleaf 템플릿과 공통 include 조각 |
| 프론트 스타일 | **SCSS 기반 공통 토큰, mixin, partial, 화면별 entry 파일** |
| 인터랙션 | Codex를 활용한 상태 변화와 사용자 반응 디자인 보완 |
| 협업 | 백엔드 개발자와 컨트롤러 연동을 고려한 화면 구조 협업 |
| 접근성 | 40~70대 사용자를 고려한 정보 전달, 조작 상태, 키보드 접근성 보완 |

## 웹 접근성 고려

돌보미의 주요 사용자는 40~70대 돌봄 종사자가 될 수 있다고 보고, 정보 인지와 조작 편의성을 함께 고려했습니다. 아이콘만으로 의미를 전달하기보다 텍스트를 함께 보여주고, 버튼과 입력 요소의 hover/focus/active 상태를 분명히 표현해 현재 조작 위치와 결과를 파악하기 쉽게 만들었습니다.

특히 마우스 hover에만 의존하지 않도록 키보드 포커스 스타일을 SCSS에 반복적으로 반영했습니다. 메뉴, 로그인 폼, 검색, 뉴스, 모달, 채용 화면 등에서 `:focus-visible` 상태를 따로 다루고, 공통 focus 표현은 mixin으로 묶어 입력 요소와 버튼의 반응을 일정하게 유지했습니다.

HTML 템플릿에서도 동적 UI 상태를 보조기술이 파악할 수 있도록 접근성 속성을 함께 사용했습니다.

| 고려 항목 | 적용 방식 |
| --- | --- |
| 현재 조작 위치 | 버튼, 링크, 입력 요소에 `:focus-visible` 상태 스타일 적용 |
| 동적 메뉴 상태 | 전체 메뉴와 필터에 `aria-expanded`, `aria-hidden` 사용 |
| 모달 의미 전달 | `role="dialog"`, `aria-modal`, 닫기 버튼 `aria-label` 사용 |
| 화면 변화 전달 | 주요 content 영역에 `aria-live` 적용 |
| 이미지와 아이콘 | 이미지 `alt` 제공, 장식 아이콘은 `aria-hidden` 처리 |
| 조작 명확성 | 아이콘 단독 노출을 줄이고 메뉴명, 버튼명, 전화번호 등 텍스트 정보 병행 |
